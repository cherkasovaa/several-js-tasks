...
const tasks = require('../src/core-js-101/05-objects-tasks');
...
describe('06-objects-tasks', () => {
  it.optional('Rectangle constructor should return the rectangle object', () => {
    ...
    assert.equal(typeof rect, 'object', 'Result of Rectangle constructor should be an object');
    assert(
  ...
  it.optional('fromJSON should return the object of specified type from JSON representation', () => {
    ...
    [
      ...
      assert.deepEqual(actual, data.expected, 'fromJson method shoud restore all properties from json');
      assert.equal(
  ...
  it.optional('cssSelectorBuilder should creates css selector object with stringify() method', () => {
    ...
    assert.equal(builder.pseudoElement('first-letter').stringify(), '::first-letter');

    // Test complex selectors
    assert.equal(builder.element('li').id('main').stringify(), 'li#main');
    assert.equal(builder.element('div').class('container').stringify(), 'div.container');
    assert.equal(builder.element('div').class('container').class('clickable').stringify(), 'div.container.clickable');
    assert.equal(builder.id('main').class('container').class('editable').stringify(), '#main.container.editable');
    assert.equal(builder.element('li').id('home-menu').class('active').stringify(), 'li#home-menu.active');
    assert.equal(
      builder.class('container').class('nav-bar').class('navbar-inverted').stringify(),
      '.container.nav-bar.navbar-inverted',
    );
    assert.equal(builder.element('a').attr('href$=".png"').pseudoClass('focus').stringify(), 'a[href$=".png"]:focus');
    assert.equal(
    ...
    assert.equal(
      builder
        .combine(builder.element('p').pseudoClass('focus'), '>', builder.element('a').attr('href$=".png"'))
        .stringify(),
      'p:focus > a[href$=".png"]',
    );
    ...
    assert.equal(
      builder
        .combine(builder.element('p').id('introduction'), '~', builder.element('img').attr('href$=".png"'))
        .stringify(),
      'p#introduction ~ img[href$=".png"]',
    );

    assert.equal(
      builder.combine(builder.id('charter1').class('touch'), '+', builder.element('table')).stringify(),
      '#charter1.touch + table',
    );

    assert.equal(
      builder
        .combine(builder.element('ul').class('animable'), ' ', builder.element('li').pseudoClass('nth-of-type(1)'))
        .stringify(),
    ...
    [
      ...
      assert.doesNotThrow(fn, /Element, id and pseudo-element should not occur more then one time inside the selector/);
    });
