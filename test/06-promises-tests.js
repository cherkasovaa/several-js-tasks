...
const tasks = require('../src/core-js-101/06-promises-tasks');
...
describe('07-promises-tasks', () => {
  it.optional(
    ...
    (done) => {
      ...
      assert(answer1 instanceof Promise && answer2 instanceof Promise, 'willYouMarryMe should return Promise!');
    ...
    (done) => {
      ...
      const result2 = tasks.processAllPromises(
        arrayForPromise.map((item) =>
          item % 2 ? Promise.resolve(item) : Promise.reject(Error(`Predictable Rejection ${item}`)),
        ),
      );
      ...
      assert(result1 instanceof Promise && result2 instanceof Promise, 'processAllPromises should return Promise!');

      // result 1
      result1
        .then((array) => {
          array.forEach((item, idx) => {
            assert.equal(item, arrayForPromise[idx], "array's items shouldn't be Promises objects!");
          });
        })
        .catch((error) => {
          const errorMessage = error instanceof Error ? error : defaultRejectionMessage;
  ...
  it.optional(
    ...
    (done) => {
      ...
      assert(result1 instanceof Promise && result2 instanceof Promise, 'getFastestPromise should return Promise!');

      // result 1
      result1
        .then((value) => {
          assert.equal(value, "I'm first!", 'getFastestPromise should return Promise that was resolved first!');
        })
        .catch((error) => {
          const errorMessage = error instanceof Error ? error : defaultRejectionMessage;
          messages.push(errorMessage.message);
        ...
        .then(() => result2)
        .then(() => {
          assert(false, 'should reject with Error was given from the first rejected Promise in array!');
        })
        .catch((error) => {
          const errorMessage = error instanceof Error ? error : defaultRejectionMessage;
          assert.equal(
  ...
  it.optional(
    ...
    (done) => {
      ...
      assert(result1 instanceof Promise && result2 instanceof Promise, 'chainPromises should return Promise!');

      // result 1
      result1
        .then((value) => {
          assert.equal(value instanceof Promise, false, "result value shouldn't be Promise object!");
          assert.equal(Array.isArray(value), false, "result value shouldn't be Array!");
          assert.equal(
            value,
            lorem,
        ...
        .catch((error) => {
          ...
          assert(errorMessage.message.match('Predictable Rejection'), 'should not reject with Errors!');
        })
        .catch((error) => {
          const errorMessage = error instanceof Error ? error : defaultRejectionMessage;
          messages.push(errorMessage.message);
