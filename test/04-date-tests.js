...
const tasks = require('../src/core-js-101/04-date-tasks');
...
describe('04-date-tasks', () => {
  it.optional('parseDataFromRfc2822 should parse rfc2822 string into a date value', () => {
    ...
    assert.equal(tasks.parseDataFromRfc2822('Tue, 26 Jan 2016 13:48:02 GMT').valueOf(), 1453816082000);

    assert.equal(tasks.parseDataFromRfc2822('Sun, 17 May 1998 03:00:00 GMT+0100').valueOf(), 895370400000);
  });

  it.optional('parseDataFromIso8601 should parse ISO 8601 string into a date value', () => {
    assert.equal(tasks.parseDataFromIso8601('2016-01-19T16:07:37+00:00').valueOf(), 1453219657000);

    assert.equal(tasks.parseDataFromIso8601('2016-01-19T08:07:37Z').valueOf(), 1453190857000);
  });
  ...
  it.optional('timeSpanToString should return the string represation of time span between two dates', () => {
    [
      ...
      assert.equal(tasks.timeSpanToString(data.startDate, data.endDate), data.expected);
    });
  });
