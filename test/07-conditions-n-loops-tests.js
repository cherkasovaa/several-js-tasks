...
const tasks = require('../src/core-js-101/07-conditions-n-loops-tasks');
...
describe('08-conditions-n-loops-tasks', () => {
  it.optional('getFizzBuzz should return the output value according specification', () => {
    [
      1, 2, 4, 7, 8, 11, 13, 14, 16, 17, 19, 22, 23, 26, 28, 29, 31, 32, 34, 37, 38, 41, 43, 44, 47, 49, 52, 53, 56, 58,
      59, 61, 62, 64, 67, 68, 71, 73, 74, 76, 77, 79, 82, 83, 86, 88, 89, 91, 92, 94, 97, 98,
    ].forEach((num) => {
      const actual = tasks.getFizzBuzz(num);
      assert.equal(actual, num, `getFizzBuzz shoud return ${num} for ${num}, but actually ${actual}`);
    });

    [3, 6, 9, 12, 18, 21, 24, 27, 33, 36, 39, 42, 48, 51, 54, 57, 63, 66, 69, 72, 78, 81, 84, 87, 93, 96, 99].forEach(
      (num) => {
        const actual = tasks.getFizzBuzz(num);
        assert.equal(actual, 'Fizz', `getFizzBuzz shoud return 'Fizz' for ${num}, but actually ${actual}`);
      },
    );

    [5, 10, 20, 25, 35, 40, 50, 55, 65, 70, 80, 85, 95, 100].forEach((num) => {
      const actual = tasks.getFizzBuzz(num);
      assert.equal(actual, 'Buzz', `getFizzBuzz shoud return 'Buzz' for ${num}, but actually ${actual}`);
    });

    [15, 30, 45, 60, 75, 90].forEach((num) => {
      const actual = tasks.getFizzBuzz(num);
      assert.equal(actual, 'FizzBuzz', `getFizzBuzz shoud return 'FizzBuzz' for ${num}, but actually ${actual}`);
    });
  });
  ...
  it.optional('getSumBetweenNumbers should return the sum inside the specified interval', () => {
    [
      ...
      assert.equal(actual, data.expected, `Sum of [${data.n1},${data.n2}] = ${data.expected}, but actual ${actual}`);
    });
  });
  ...
  it.optional('reverseString should return the specified string in reverse order', () => {
    [
      ...
      assert.equal(actual, data.expected, `Reversed '${data.str}' = '${data.expected}', but actual '${actual}'`);
    });
  });
  ...
  it.optional('reverseInteger should return the specified number in reverse order', () => {
    [
      ...
      assert.equal(actual, data.expected, `Reversed ${data.num} = ${data.expected}, but actual ${actual}`);
    });
  });

  it.optional('isCreditCardNumber should validate CCN', () => {
    [
      79927398713, 4012888888881881, 5123456789012346, 378282246310005, 371449635398431, 378734493671000,
      5610591081018250, 30569309025904, 38520000023237, 6011111111111117, 6011000990139424, 3530111333300000,
      3566002020360505, 5555555555554444, 5105105105105100, 4111111111111111, 4012888888881881, 4222222222222,
      5019717010103742, 6331101999990016, 54891243456789010,
    ].forEach((ccn) => {
      assert(tasks.isCreditCardNumber(ccn), `CCN ${ccn} is valid, but actually not`);
    });

    [4571234567890111, 5436468789016589, 4916123456789012, 371449635398430, 9112893456789010].forEach((ccn) => {
  ...
  it.optional('getDigitalRoot should return the cyclic sum of all digits', () => {
    [
      ...
      assert.equal(actual, data.expected, `GetDigitalRoot(${data.num}) = ${data.expected}, but actual ${actual}`);
    });
  });
  ...
  it.optional('isBracketsBalanced should check the balanced brackets', () => {
    ...
    [
      ...
      assert(tasks.isBracketsBalanced(str) === false, `'${str}' has unbalanced brackets, but actually yes`);
    });
  });
  ...
  it.optional('toNaryString should return the n-ary string representation of number', () => {
    [
      ...
      assert.equal(actual, data.expected, `${data.num} with radix ${data.n} = ${data.expected}, but actual ${actual}`);
    });
  });
